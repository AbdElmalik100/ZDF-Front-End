...
function Attendance() {
    ...
    useEffect(() => {
        if (isLoggedIn) {
            setLoading(true)
            axios.post(`api/attendances/${params.id}/submit_attendance`, user)
                .then(response => {
                    setAlreadySubmitted(false)
                    socket.emit("submit_attendance", response.data)
                }).catch(error => {
                    console.log(error);
                        ...
                        <div className="container h-screen px-4 grid place-items-center">
                            ...
                            >
                                ...
                                <h3 className={`font-semibold ${alreadySubmitted ? "text-rose-700" : "text-green-700"}`}>
                                    {
                                        alreadySubmitted ? "Your have already submitted your attendance before!" : "Thanks for your time, your attendance has been checked out, Enjoy."
                                    }
                                </h3>
